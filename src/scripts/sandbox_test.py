...
saver = tf.train.import_meta_graph('models/20180718_181706/AT-LSTM.meta')
saver.restore(sess, tf.train.latest_checkpoint('models/20180718_181706'))
...
logger.info('Predicting on dev...')
batch_generator = dm.batch_generator(dev_df, -1)
...
logger.info('Dev   acc/3={:.2%}'.format(accuarcy_))

logger.info('Predicting on train...')
batch_generator = dm.batch_generator(test_df, -1)
_, batch = next(batch_generator)
_X, _asp, _lx, _y = batch
alpha_, pred_, accuarcy_ = sess.run([alpha, pred, accuarcy], feed_dict={X: _X, asp: _asp, y: _y})
logger.info('Train acc/3={:.2%}'.format(accuarcy_))
