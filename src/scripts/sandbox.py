...
dropout_keep = tf.placeholder(tf.float32, shape=(1, ), name='dropout_keep')
...
with tf.name_scope('Embedding'):
    ...
    asp_ = tf.nn.embedding_lookup(embedding, asp) # (batch, 1, d)
...
WaVaeN = matmul_2_3(Wv, VaeN_T) # (batch, da, N)




# _H = tf.transpose(H, [2, 1, 0]) # (d, N, batch)
# __H = tf.reshape(_H, (_H.shape[0], -1)) # (d, N*batch)
#
# Wh = tf.get_variable('Wa', shape=(glove.shape[1], glove.shape[1]), dtype=tf.float32)
#
# _WhH = tf.matmul(Wh, __H)
# WhH = tf.reshape(_WhH, (_H.shape[0], -1, hyparams['batch_size']))
